  ...
  return (
    <section className="bg-background" style={{ paddingTop: SPACING.sectionPy.desktop, paddingBottom: SPACING.sectionPy.desktop }}>
      <div className="mx-auto" style={{ maxWidth: SPACING.maxWidth, paddingLeft: SPACING.sectionPx, paddingRight: SPACING.sectionPx }}>
        <h2 style={{ ...TYPE.h2, color: "#001965", marginBottom: "56px" }} className="max-w-3xl">
          Enhance biosimilar peptide development with AMOGEN's proprietary platform
        </h2>

        <div className="rounded-2xl p-8 md:p-12 lg:p-16" style={{ backgroundColor: "#F0EFED" }}>
          <div className="grid md:grid-cols-[minmax(280px,380px)_1fr] gap-10 lg:gap-16">
            <div className="flex flex-col gap-2">
              {items.map((item, i) => {
                const isActive = active === i;
                return (
                  <button key={i} onClick={() => setActive(i)} className={`text-left px-5 py-5 rounded-lg transition-all duration-300 flex items-center justify-between gap-4 ${isActive ? "bg-white shadow-sm" : "bg-transparent"}`}>
                    <span style={{ ...TYPE.label, color: "#001965", opacity: isActive ? 1 : 0.5 }}>{item.label}</span>
                    {isActive && (
                      <motion.span initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }}>
            ...
            <div className="min-h-[280px] flex items-start">
              <AnimatePresence mode="wait">
                <motion.div key={active} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.35, ease: "easeOut" }}>
                  <h3 style={{ ...TYPE.h3, color: "#001965", marginBottom: "24px" }}>{items[active].heading}</h3>
                  <p style={{ ...TYPE.body, color: "hsl(var(--muted-foreground))" }}>{items[active].body}</p>
                </motion.div>
              </AnimatePresence>
            </div>
          </div>
